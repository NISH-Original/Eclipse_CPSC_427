...
Entity createBubble(RenderSystem* renderer, vec2 position)
{
	// Reserve en entity
	auto entity = Entity();
...
Entity createPinkFish(RenderSystem* renderer, vec2 position)
{
	// Reserve en entity
	auto entity = Entity();
	...
	// Create an (empty) Bug component to be able to refer to all bug
	registry.eatables.emplace(entity);
	registry.pinkFishes.emplace(entity);
	registry.renderRequests.insert(
		entity,
		{
			TEXTURE_ASSET_ID::PINK_FISH,
			EFFECT_ASSET_ID::TEXTURED,
			GEOMETRY_BUFFER_ID::SPRITE
		});
