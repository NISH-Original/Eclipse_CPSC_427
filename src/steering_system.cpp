...
static void add_steering() {
    ...
    for (int i = 0; i < dirs_registry.components.size(); i++) {
        const AccumulatedForce& af = dirs_registry.components[i];
        const Entity& e = dirs_registry.entities[i];
        if (steering_registry.has(e)) {
            steering_registry.get(e).target_angle = glm::atan(af.v.y, af.v.x);
        } else {
            registry.enemy_steerings.insert(e, { glm::atan(af.v.y, af.v.x) });
        }
    }
}

static void update_rotate(float elapsed_ms) {
